...
const defaultLibraryState: LibraryState = { workspaces: defaultWorkspaces, memberships: { "hank-ftpl": ["auclert", "kaplan", "cochrane", "bassetto"] }, hiddenPaperIds: [], highlights: {}, favoritePaperIds: [], recentPaperIds: [] };
...
export default function Home() {
  ...
  const visiblePapers = useMemo(() => { const term = query.trim().toLowerCase(); return term ? papersInView.filter((paper) => [paper.title, paper.authors, paper.tags.join(" ")].join(" ").toLowerCase().includes(term)) : papersInView; }, [papersInView, query]);
  const commandResults = useMemo(() => { const term = query.trim().toLowerCase(); return term ? availablePapers.filter((paper) => [paper.title, paper.authors, paper.tags.join(" ")].join(" ").toLowerCase().includes(term)) : availablePapers; }, [availablePapers, query]);
  ...
  useEffect(() => {
    ...
    if (desktopApp) {
      Promise.all([loadDesktopPapers(), loadDesktopNotes(), loadDesktopLibraryState()]).then(([imported, savedNotes, state]) => {
        setLibraryPapers((current) => [...current, ...imported.filter((paper) => !current.some((item) => item.id === paper.id))].map((paper) => ({ ...paper, highlights: [...paper.highlights, ...(state.highlights[paper.id] ?? [])] })));
      ...
      readLocalPapers().then((imported) => setLibraryPapers((current) => [...current, ...imported.filter((paper) => !current.some((item) => item.id === paper.id))].map((paper) => ({ ...paper, highlights: [...paper.highlights, ...(state.highlights[paper.id] ?? [])] })))).catch(() => undefined);
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "Escape") { setCommandOpen(false); setMenu(null); setHighlightOpen(false); setCreateWorkspaceOpen(false); }
    };
